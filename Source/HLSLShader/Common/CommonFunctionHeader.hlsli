...
/// @param power 強さ
/// @return 
float CalculateSpecular(float3 lightRay, float3 normal, float3 ray, float power)
{
    return pow(saturate(dot(-ray, reflect(lightRay, normal))), power);
}
...
void ProcessPointLight(float3 pos, float3 ray, float3 normal, Light light, Material material, inout float3 totalDiffuse, inout float3 totalSpecular)
{
	//ライトから位置へのベクトルを算出
